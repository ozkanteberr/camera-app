...
class ARSessionManager {
  ...
  /// Returns the world transforms where normalized screen points hit a tracked AR plane.
  Future<List<Matrix4>?> hitTestPlaneQuad(List<Offset> normalizedPoints) async {
    try {
      final serializedHits = await _channel.invokeMethod<List<dynamic>>(
        'hitTestPlaneQuad',
        {
      ...
      return serializedHits
          .map((hit) => MatrixConverter().fromJson(hit as List))
          .toList(growable: false);
    } catch (e) {
      print('Error caught: ' + e.toString());
      return null;
  ...
  Future<Map<String, Object>?> hitTestPlaneViewport({
    int columns = 9,
    int rows = 13,
    double horizontalMargin = 0.04,
    double verticalMargin = 0.06,
  }) async {
    try {
      ...
      final rectValues = (result['rect'] as List?)?.cast<num>();
      final serializedHits = result['hits'] as List?;
      if (rectValues == null ||
          rectValues.length != 4 ||
          serializedHits == null ||
          serializedHits.length != 4) {
        return null;
      }

      final hits = serializedHits
          .map((hit) => MatrixConverter().fromJson(hit as List))
          .toList(growable: false);
      return {
        'rect': Rect.fromLTRB(
          rectValues[0].toDouble(),
          rectValues[1].toDouble(),
          rectValues[2].toDouble(),
          rectValues[3].toDouble(),
        ),
        'hits': hits,
      };
    } catch (e) {
  ...
  Future<void> setPlaneDetectionEnabled(bool enabled) async {
    try {
      await _channel.invokeMethod<void>('setPlaneDetectionEnabled', {
        'enabled': enabled,
      });
    } catch (e) {
      print('Error caught: ' + e.toString());
