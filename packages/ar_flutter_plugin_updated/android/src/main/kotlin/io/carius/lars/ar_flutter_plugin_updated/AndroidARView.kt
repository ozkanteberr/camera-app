...
internal class AndroidARView(
    ...
    private var configuredPlaneFindingMode = Config.PlaneFindingMode.HORIZONTAL_AND_VERTICAL
    private var planeDetectionCallbacksEnabled = true
            ...
            object : MethodChannel.MethodCallHandler {
                override fun onMethodCall(call: MethodCall, result: MethodChannel.Result) {
                    ...
                    when (call.method) {
                        ...
                        "hitTestPlaneQuad" -> {
                            result.success(hitTestPlaneQuad(call))
                        }
                        "hitTestPlaneViewport" -> {
                            result.success(hitTestPlaneViewport(call))
                        }
                        "snapshot" -> {
                        ...
                        "setPlaneDetectionEnabled" -> {
                            setPlaneDetectionEnabled(call.argument<Boolean>("enabled") ?: true)
                            result.success(null)
                        }
                        else -> {}
                    }
...
//                        onDestroy()
    ...
    private fun initializeARView(call: MethodCall, result: MethodChannel.Result) {
        ...
        configuredPlaneFindingMode = when (argPlaneDetectionConfig) {
        ...
        planeDetectionCallbacksEnabled = configuredPlaneFindingMode != Config.PlaneFindingMode.DISABLED
        config?.planeFindingMode = configuredPlaneFindingMode
        arSceneView.session?.configure(config)
    ...
    private fun onFrame(frameTime: FrameTime) {
        if(isCameraEnabled){
        if (planeDetectionCallbacksEnabled && arSceneView.arFrame != null){
    ...
    private fun setPlaneDetectionEnabled(enabled: Boolean) {
        planeDetectionCallbacksEnabled = enabled
        if (enabled) {
            val config = arSceneView.session?.config ?: return
            config.planeFindingMode = configuredPlaneFindingMode
            arSceneView.session?.configure(config)
        }
    }

    private fun hitTestPlaneAt(frame: Frame, normalizedX: Float, normalizedY: Float, requiredPlane: Plane? = null): HitResult? {
        if (arSceneView.width <= 0 || arSceneView.height <= 0) return null
        val screenX = normalizedX.coerceIn(0.0f, 1.0f) * arSceneView.width.toFloat()
        val screenY = normalizedY.coerceIn(0.0f, 1.0f) * arSceneView.height.toFloat()
        return frame.hitTest(screenX, screenY).firstOrNull { hitResult ->
            ...
            if (!trackable.isPoseInPolygon(hitResult.hitPose)) return@firstOrNull false
            requiredPlane == null || requiredPlane == trackable
        }
    }

    private fun hitTestPlaneViewport(call: MethodCall): HashMap<String, Any>? {
        ...
        val marginX = ((call.argument<Any>("horizontalMargin") as? Number)?.toFloat() ?: 0.04f).coerceIn(0.0f, 0.25f)
        val marginY = ((call.argument<Any>("verticalMargin") as? Number)?.toFloat() ?: 0.06f).coerceIn(0.0f, 0.25f)
        val leftLimit = marginX
        val rightLimit = 1.0f - marginX
        val topLimit = marginY
        val bottomLimit = 1.0f - marginY
        if (rightLimit <= leftLimit || bottomLimit <= topLimit) return null

        val stepX = (rightLimit - leftLimit) / (columns - 1).toFloat()
        val stepY = (bottomLimit - topLimit) / (rows - 1).toFloat()
        ...
        for (row in 0 until rows) {
            ...
            for (column in 0 until columns) {
                ...
                val hit = hitTestPlaneAt(frame, x, y) ?: continue
                val plane = hit.trackable as Plane
                var points = hitsByPlane[plane]
                if (points == null) {
                    points = mutableListOf()
                    hitsByPlane[plane] = points
                }
                points.add(Pair(x, y))
            }
        }
        ...
        var selectedPoints: MutableList<Pair<Float, Float>>? = null
        for (entry in hitsByPlane.entries) {
            if (selectedPoints == null || entry.value.size > selectedPoints!!.size) {
                selectedPlane = entry.key
                selectedPoints = entry.value
            }
        }

        val plane = selectedPlane ?: return null
        val points = selectedPoints ?: return null
        ...
        val shrinkSteps = floatArrayOf(0.0f, 0.015f, 0.03f, 0.05f, 0.075f, 0.10f)
        for (shrink in shrinkSteps) {
            val l = (baseLeft + shrink).coerceIn(leftLimit, rightLimit)
            val r = (baseRight - shrink).coerceIn(leftLimit, rightLimit)
            val t = (baseTop + shrink).coerceIn(topLimit, bottomLimit)
            val b = (baseBottom - shrink).coerceIn(topLimit, bottomLimit)
            if (r - l < 0.12f || b - t < 0.12f) continue

            val rectPoints = listOf(Pair(l, t), Pair(r, t), Pair(r, b), Pair(l, b))
            val serializedHits = ArrayList<DoubleArray>()
            var allCornersHit = true
            for (point in rectPoints) {
                val planeHit = hitTestPlaneAt(frame, point.first, point.second, plane)
                if (planeHit == null) {
                    allCornersHit = false
                    break
                }
                serializedHits.add(serializePose(planeHit.hitPose))
            }

            if (allCornersHit) {
                val output = HashMap<String, Any>()
                output["rect"] = arrayListOf(l.toDouble(), t.toDouble(), r.toDouble(), b.toDouble())
                output["hits"] = serializedHits
                return output
            }
        }
    ...
    private fun hitTestPlaneQuad(call: MethodCall): ArrayList<DoubleArray>? {
        val frame = arSceneView.arFrame ?: return null
        val rawPoints = call.argument<List<Map<String, Any>>>("points") ?: return null
        if (rawPoints.isEmpty() || arSceneView.width <= 0 || arSceneView.height <= 0) {
            return null
        }
        ...
        for (point in rawPoints) {
            val normalizedX = (point["x"] as? Number)?.toFloat() ?: return null
            val normalizedY = (point["y"] as? Number)?.toFloat() ?: return null
            val planeHit = hitTestPlaneAt(frame, normalizedX, normalizedY, selectedPlane) ?: return null

            selectedPlane = planeHit.trackable as Plane
            serializedHits.add(serializePose(planeHit.hitPose))
        }
